...
Deploy the same file to both boxes:

```bash
scp icmptun-ctl.sh root@server:/root/icmptun-ctl.sh
ssh root@server chmod +x /root/icmptun-ctl.sh
```

Run it with no arguments to open the interactive menu — first run walks you through initial setup:
